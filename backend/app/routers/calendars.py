...
async def get_calendars(user: SessionUser = Depends(get_current_user)):
    ...
    どのカレンダーを空き時間チェック対象にするか（最大3件）・新規予定の登録先はどれか（複数可・最大3件）を
    ...
    for provider in ("google", "outlook"):
        ...
        result[provider] = {
            ...
            "writeIds": token_row.get("write_calendar_ids") or [],
        }
    return result
...
class SelectCalendarsRequest(BaseModel):
    ...
    write_calendar_ids: list[str] | None = None  # None/空リスト = 既定カレンダーへ登録

    @field_validator("calendar_ids", "write_calendar_ids")
    @classmethod
...
def select_calendars(
    ...
    set_calendar_selection(
        ...
        write_calendar_ids=body.write_calendar_ids or None,
    )
    return {"ok": True, "selectedIds": body.calendar_ids or [], "writeIds": body.write_calendar_ids or []}
